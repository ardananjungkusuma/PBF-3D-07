...
function Topik() {
  let { topicId, topic } = useParams();
  return (
    <div>
      <h3>Topik yang terpilih: </h3>
      <Card className="m-3">
        <Card.Body>
          <div>
            <h3>{topicId}</h3>
...
function Codelabs() {
  ...
  return (
    <Container>
        ...
        <Col sm={12} className="mt-5">
          ...
          <div>
            ...
            <ul>
              <li>
                <Link
                  to={`${match.url}/konsep-reactjs/Stateless, statefull, dynamic component, lifecycle component`}
                >
                  Konsep ReactJS
                </Link>
              </li>
              <li>
                <Link
                  to={`${match.url}/belajar-react-router/Mahasiswa Akan Mempelajari berpindah antara halaman dengan react router, merapikan struktur folder dan rename browse router, mengirimkan params ke halaman details router`}
                >
      ...
      <Switch>
        <Route path={`${match.path}/:topicId/:topic`}>
          <Topik />
        </Route>
